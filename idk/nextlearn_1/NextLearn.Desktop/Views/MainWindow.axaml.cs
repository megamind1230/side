...
public partial class MainWindow : Window
{
    ...
    private void UpdateEditor()
    {
        ...
        var editor = this.FindControl<TextEditor>("ContentEditor");
        if (editor == null) return;

        editor.Text = vm.LearningViewModel.RenderedContent;
        SetupEditor(editor, vm.LearningViewModel.IsOrgFile);
    }

    private void SetupEditor(TextEditor editor, bool isOrg)
    {
        _textMateInstallation?.Dispose();

        var registry = new RegistryOptions(ThemeName.DarkPlus);
        _textMateInstallation = editor.InstallTextMate(registry);
